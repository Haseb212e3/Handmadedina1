    ...
    <header className="relative z-50 w-full px-4 py-4">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          {languages.map((lang) => (
            <button
              ...
              className={`text-2xl transition-all duration-300 hover:scale-110 ${
                currentLang === lang.code ? 'opacity-100 scale-110' : 'opacity-70'
              }`}
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navigation[currentLang as keyof typeof navigation].map((item) => (
            <a
              key={item.name}
              href={item.href}
      ...
      {/* Mobile Navigation */}
      {isMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-card/90 backdrop-blur-sm border-t border-border">
          <nav className="flex flex-col py-4">
            {navigation[currentLang as keyof typeof navigation].map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="px-6 py-3 text-foreground hover:text-accent hover:bg-muted/50 transition-all duration-300"
